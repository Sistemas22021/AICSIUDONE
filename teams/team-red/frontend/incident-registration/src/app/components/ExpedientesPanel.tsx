...
interface ExpedientesPanelProps {
    ...
    modo?: 'busqueda' | 'sellado'
    /** Notificación global (compartida con el resto de la app) — el panel no crea su propio sistema de toasts. */
...
export const ExpedientesPanel = ({
    ...
    // En modo sellado: exactamente los expedientes en BORRADOR, sin importar su antigüedad.
    const expedientesFiltrados = modo === 'sellado'
    ...
    const totalBorrador = expedientes.filter(e => e.estatus === 'BORRADOR').length
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
                ...
                <div className="flex-shrink-0 px-6 pt-5 pb-4 border-b border-cyan-400/20">
                    <div className="flex items-start justify-between gap-4">
                        ...
                        <div>
                            ...
                            >
                                {modo === 'sellado' ? 'Sellar Expediente' : 'Expedientes Activos'}
                            </div>

                            <div className="flex flex-wrap items-center gap-3">
                                {modo === 'sellado' ? (
                                    <div
                                        className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-cyan-500">
                                        <span className="inline-block w-2 h-2 rounded-full bg-amber-400"
                                              style={{boxShadow: '0 0 6px rgba(255,170,0,0.7)'}}/>
                                        {totalBorrador} en borrador — disponibles para sellar
                                    </div>
                                ) : (
                                    <div
                                        className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-cyan-500">
                                ...
                                {totalAlertas > 0 && (
                                    <div
                                        className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-amber-400">
                                        <AlertTriangle size={11}
                                                       style={{filter: 'drop-shadow(0 0 4px rgba(255,170,0,0.7))'}}/>
                                ...
                                {/* Estado en vivo */}
                                <div
                                    className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-cyan-600">
                                    <Dot
                                        size={16}
                                ...
                                {usingMock && (
                                    <div
                                        className="flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-amber-400/70">
                ...
                {/* ══ CONTENIDO PRINCIPAL (conmutado entre lista y formulario de sellado) ══ */}
                {vista === 'sellar' && expedienteASellar ? (
                    <div className="flex-1 overflow-auto px-6">
                        <SellarExpedienteForm
                            expediente={expedienteASellar}
                            onCancelar={() => {
                                setVista('lista');
                                setExpedienteASellar(null)
                            }}
                            onSellado={() => {
                                showToast('Expediente sellado exitosamente.', 'success')
                                refetch()
                                setVista('lista')
                                setExpedienteASellar(null)
                            }}
                    ...
                    <>
                        ...
                        <div className="flex-shrink-0 px-6 py-3 border-b border-cyan-400/15 bg-[#04101E]/40">
                            <div className="flex flex-wrap gap-3 items-end">
                                ...
                                {/* Filtro estatus — solo tiene sentido en modo búsqueda */}
                                {modo !== 'sellado' && (
                                    <div className="w-40">
                                        <NeonSelect
                                            options={[
                                ...
                                {/* Toggle solo alertas — no aplica en modo sellado (BORRADOR no tiene MO todavía) */}
                                {modo !== 'sellado' && (
                                    <button
                ...
                : 'No se encontraron expedientes con los filtros aplicados'}
                        ...
                        />
                                                ...
                                                <td className="py-3 px-3">
                                                    <div
                                                        className="text-cyan-300 leading-tight">{exp.subtipoDelito ?? 'Sin especificar'}</div>
                                                ...
                                                <td className="py-3 px-3 text-cyan-400 whitespace-nowrap">
                                                    ...
                                                    <div
                                                        className="text-[10px] text-cyan-600 mt-0.5">{exp.municipio}</div>
                                                </td>
                        ...
                        {estiloEstatus.label}
                                                ...
                                                <td className="py-3 px-3 text-center">
                                                    {modo === 'sellado' ? (
                                                        <button
                                                            onClick={() => {
                                                                setExpedienteASellar(exp)
                                                                setVista('sellar')
                                                            }}
                                                            disabled={exp.estatus !== 'BORRADOR'}
                                                            className={[
                                                                'px-3 py-1 rounded-sm text-[10px] uppercase tracking-wider transition-all',
                                                                exp.estatus !== 'BORRADOR'
                                                                    ? 'border border-gray-500/30 text-gray-500 cursor-not-allowed'
                                                                    : 'border border-emerald-400/40 text-emerald-400 hover:bg-emerald-400/15 hover:border-emerald-400 hover:text-emerald-300'
                                                            ].join(' ')}
                                                            style={{boxShadow: '0 1px 4px rgba(0,255,136,0.15)'}}
                                                        >
                                                            {exp.estatus !== 'BORRADOR' ? 'No disponible' : 'Sellar'}
                                                        </button>
                                                    ) : (
                                                        <button
